...
class SerialConnection:
    ...
    #일산화탄소 연결 메서드
    def co_serial_communication(self, port_name):
        # 각 커맨드와 그에 해당하는 URL을 매핑한 이중 배열
        self.commands = b"\x0A\x03\x00\xCB\x00\x02\xB4\x8E"
        try:
            ...
            if port.is_open:
                ...
                # 스프링 서버로 실수값 데이터 전송
                response = requests.post("http://localhost:8080/CoConnection", json={'data': float_value})
    ...
    #백엽상 연결 메서드
    def inshlt_serial_communication(self, port_name):
        # 각 커맨드와 그에 해당하는 URL을 매핑한 이중 배열
        self.commands = [
        ...
        try:
            ...
            if port.is_open:
                ...
                # 스프링 서버로 실수값 데이터 전송
                response = requests.post("http://localhost:8080/inshltConnection", json={'data': self.float_values})
                print("Data sent to Spring server:", response.text)
...
class SerialServer:
    def __init__(self, port_name):
        ...
        self.co_thread = None
        self.inshlt_thread = None
        self.is_running = False

    def start(self):
        if self.is_running:
            print("Serial server is already running.")
            return
        ...
        try:
            while self.is_running:
                if self.co_thread is None or not self.co_thread.is_alive():
                    self.co_thread = threading.Thread(target=self.serial_connection.co_serial_communication, args=(self.port_name,))
                    self.co_thread.start()

                if self.inshlt_thread is None or not self.inshlt_thread.is_alive():
                    self.inshlt_thread = threading.Thread(target=self.serial_connection.inshlt_serial_communication, args=(self.port_name,))
                    self.inshlt_thread.start()

                sleep(10)  # 10 seconds delay before starting the next iteration
        except Exception as e:
